...
class NCRPNode(object):
    ...
    def add_child(self):
        ...
        node = NCRPNode(self.num_levels, self.vocab, parent=self, level=self.level+1, random_state=self.random_state)
...
class HierarchicalLDA(object):
    ...
    def __init__(self, corpus, vocab,
        ...
        # we added earlier.
        self.root_node = NCRPNode(self.num_levels, self.vocab, random_state=self.random_state)
    ...
    def estimate(self, num_samples, display_topics=50, n_words=5, with_weights=True, path='./result/hlda/res.txt'):
        ...
        for s in tqdm(range(num_samples)):
            ...
            if (s > 0) and ((s+1) % display_topics == 0):
                ...
                with open(path, 'a') as f:
                    print(" %d" % (s+1), file=f)
                    self.print_nodes(n_words, with_weights, file=f)
                    print('', file=f)
    ...
    def print_nodes(self, n_words, with_weights, file=sys.stdout):
        self.print_node(self.root_node, 0, n_words, with_weights,  file=file)

    def print_node(self, node, indent, n_words, with_weights, file=sys.stdout):
        out = '    ' * indent
        out += 'topic=%d level=%d (documents=%d): ' % (node.node_id, node.level, node.customers)
        out += node.get_top_words(n_words, with_weights)
        print(out, file=file)
        for child in node.children:
            self.print_node(child, indent+1, n_words, with_weights, file=file)

    # TODO
    # def empiricalLikelihood(int numSamples, InstanceList testing) {
    #     path = np.zeros(self.num_levels, dtype=np.object)
    #     path[0] = self.root_node

    #     dirichlet = np.random.dirichlet([alpha] * numLevels);
    #     level_weights = np.zeros(self.num_levels)

    #     # TODO
    #     FeatureSequence fs;
    #     level_weights = np.zeros(self.num_types)
    ...
    #         levelWeights = dirichlet.transpose();
